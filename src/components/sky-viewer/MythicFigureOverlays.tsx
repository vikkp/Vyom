...
const MYTHIC_FIGURES: MythicFigureDef[] = [
  ...
  // Fifth batch: Mula, Shravana, Dhanishta. Real art has landed for all
  // three (784x1168, same portrait dimensions as the rest) -- realSize
  // below is measured from the actual delivered pixels. All three
  // compositions are close to vertically centred (alpha centroid
  // 0.468-0.501 of frame height), so none need a realAnchorFrac override.
  {
    nodeId: "mula",
...
export function MythicFigureOverlays({ stars }: MythicFigureOverlaysProps) {
  ...
  const placed = useMemo(() => {
    return MYTHIC_FIGURES.map((def) => {
      ...
      return { item: def, position };
    }).filter((v): v is { item: MythicFigureDef; position: Vector3 } => v !== null);
  }, [starsById, selectedCity, currentDate]);

  // Nudge any figures that landed too close together (see
  // declutterPositions in figureDeclutter.ts) -- most pairs are naturally
  // far enough apart that this is a no-op; it only kicks in for genuinely
  // close neighbours like Ardra/Mrigashira. The Rishi portraits go through
  // the same shared pass in RishiOverlays.tsx.
  ...
  return (
    <group>
      {declutteredPlaced.map(({ item: def, position }) => (
